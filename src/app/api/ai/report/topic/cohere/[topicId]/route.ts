  ...
  try {
      // const {topicId:receivedTopicId} = params;
       const pathname = new URL(req.url).pathname;
        ...
        // 2. Split and take the last segment
      ...
      if(!user){
        ...
        return Response.json(
          new ApiResponse(false,"Unauthorized - This topic is not owned by you",null,null,"UNAUTHORIZED"),{
            status:400
          }
        )
      } else if(!user.isVerified){
        return Response.json(
          new ApiResponse(false,"You are not verified",null,null,"UNVERIFIED"),{
            status:400
          }
        )
      }
      ...
      if(feedbacks.length===0){
        return Response.json(
          new ApiResponse(false,"No feedbacks yet on this topic",null,null,"EMPTY"),{
            status:400
          }
        )
      }
